...
zooAnimals.forEach(i => {
	animalNames.push(`Name: ${i.animal_name} Scientific name: ${i.scientific_name}`);
});
...
const lowerCase = [];
let unCap = zooAnimals.map(i => i.animal_name.toLowerCase());
lowerCase.push(unCap);
...
*/
let lowerPopulation = [];
lowerPopulation = zooAnimals.filter(i => i.population < 5);
...
*/
let populationTotal = [];
populationTotal = zooAnimals.reduce((total, representation) => {
	return (total += representation.population);
}, 0);
